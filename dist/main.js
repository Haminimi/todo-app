...
function lightFormat(dirtyDate, formatStr) {
  ...
  var timezoneOffset = getTimezoneOffsetInMilliseconds(originalDate);
  var utcDate = subMilliseconds(originalDate, timezoneOffset);
  var tokens = formatStr.match(lightFormat_formattingTokensRegExp);
...
;// CONCATENATED MODULE: ./src/check.png
const check_namespaceObject = __webpack_require__.p + "76284ccbdc6c5fb8894c.png";
;// CONCATENATED MODULE: ./src/DOM.js


const { parse, differenceInDays } = __webpack_require__(105);

const header = document.querySelector('.header');
const headerSticker = new Image();
headerSticker.classList.add('header-sticker');
headerSticker.src = check_namespaceObject;
header.appendChild(headerSticker);


function displayProject(project, todoList, projectId) {
    //A project container
...
function displayNoProjectsCard() {
...
;// CONCATENATED MODULE: ./src/createProject.js
...
;// CONCATENATED MODULE: ./src/addAndDelete.js
...
;// CONCATENATED MODULE: ./src/handleInput.js


function handleProjectTitleChange(title) {
...
function handlePriorityChangeToLow(event) {
    const projectId = event.target.getAttribute('data-project-id');
    const todoId = event.target.getAttribute('data-id');
...
function handlePriorityChangeToHigh(event) {
    const projectId = event.target.getAttribute('data-project-id');
    const todoId = event.target.getAttribute('data-id');
...
function handleExpandStateChangeToExpand(event) {
    const projectId = event.target.getAttribute('data-project-id');
    const todoId = event.target.getAttribute('data-id');
...
function handleExpandStateChangeToShrink(event) {
    const projectId = event.target.getAttribute('data-project-id');
    const todoId = event.target.getAttribute('data-id');
